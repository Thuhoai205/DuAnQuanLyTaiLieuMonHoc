@extends('layouts.admin')

@section('title', 'Quản lý môn học')
@section('page-title', 'Quản lý môn học')

@section('content')

@php
$totalSubjects = $totalSubjects ?? $subjects->total();
$totalTeachers = $totalTeachers ?? 0;
$totalDocuments = $totalDocuments ?? 0;
@endphp

<div class="space-y-6">

    <!-- HEADER -->
    <div class="bg-white border rounded-md shadow-sm p-5">
        <div class="flex justify-between items-center">

            <!-- LEFT -->
            <div>
                <h2 class="text-lg font-black">
                    Quản lý môn học
                </h2>

                <p class="text-sm text-slate-500">
                    Danh sách môn học trong hệ thống
                </p>
            </div>

            <!-- RIGHT ACTIONS -->
            <div class="flex items-center gap-3">

                <!-- TRASH BUTTON -->
                <a href="{{ route('admin.subjects.trashed') }}"
                    class="inline-flex items-center gap-2 h-11 px-4 rounded-md bg-white border border-red-200 text-red-500 text-sm font-black hover:bg-red-500 hover:text-white transition">

                    <i class="fa-solid fa-trash-can-arrow-up"></i>

                    <span>Đã xóa</span>

                    @if($totalTrashedSubjects > 0)
                    <span
                        class="min-w-6 h-6 px-2 rounded-full bg-red-500 text-white text-xs font-black flex items-center justify-center">
                        {{ $totalTrashedSubjects }}
                    </span>
                    @endif

                </a>

                <!-- CREATE BUTTON -->
                <a href="{{ route('admin.subjects.create') }}"
                    class="h-11 px-4 flex items-center bg-sky-500 text-white rounded-md font-black hover:bg-sky-600 transition">

                    <i class="fa-solid fa-plus mr-2"></i>
                    Thêm môn học

                </a>

            </div>

        </div>
    </div>
    <!-- FILTER -->
    <div class="bg-white border border-slate-200 rounded-md shadow-sm p-5 mb-6">

        <form id="subjects-filter-form" method="GET" action="{{ route('admin.subjects.index') }}"
            class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">

            <!-- SEARCH -->
            <div class="md:col-span-5 relative">
                <i class="fa-solid fa-magnifying-glass absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i>

                <input type="text" name="search" value="{{ request('search') }}"
                    placeholder="Tìm theo mã môn, tên môn..." class="w-full h-11 pl-11 pr-4 rounded-md bg-slate-50 border border-slate-200 text-sm font-semibold
                          focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500">
            </div>

            <!-- FACULTY -->
            <div class="md:col-span-3">
                <select name="faculty_id" class="w-full h-11 px-4 rounded-md bg-slate-50 border border-slate-200 text-sm font-semibold
                           focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500">

                    <option value="">Tất cả khoa</option>

                    @foreach($faculties as $faculty)
                    <option value="{{ $faculty->faculty_id }}" @selected(request('faculty_id')==$faculty->faculty_id)>
                        {{ $faculty->faculty_name }}
                    </option>
                    @endforeach

                </select>
            </div>

            <!-- STATUS -->
            <div class="md:col-span-2">
                <select name="status" class="w-full h-11 px-4 rounded-md bg-slate-50 border border-slate-200 text-sm font-semibold
                           focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500">

                    <option value="">Trạng thái</option>
                    <option value="active" @selected(request('status')=='active' )>Hoạt động</option>
                    <option value="archived" @selected(request('status')=='archived' )>Ẩn</option>

                </select>
            </div>

            <!-- BUTTON FILTER -->
            <div class="md:col-span-1">
                <button type="submit" class="w-full h-11 rounded-md bg-sky-500 text-white text-sm font-black
                           hover:bg-sky-600 transition flex items-center justify-center gap-2">

                    <i class="fa-solid fa-filter text-xs"></i>
                    Lọc
                </button>
            </div>

            <!-- RESET -->
            <div class="md:col-span-1">
                <a href="{{ route('admin.subjects.index') }}" id="reset-subject-filter" class="w-full h-11 rounded-md bg-slate-100 text-slate-600 text-sm font-black
                      flex items-center justify-center hover:bg-slate-200 transition">

                    Reset
                </a>
            </div>

        </form>
    </div>
    <!-- LIST -->
    <!-- USER SUMMARY CARD -->
    <div class="bg-white border border-slate-200 rounded-md shadow-sm overflow-hidden mb-6">

        <!-- HEADER -->
        <div class="px-5 py-4 border-b border-slate-200 flex items-center justify-between">

            <div>
                <h2 class="text-sm font-black text-slate-700">
                    Danh sách môn học
                </h2>

                <p class="text-xs text-slate-400 font-semibold mt-1">
                    Hiển thị thông tin môn học, khoa, giảng viên và tài liệu.
                </p>
            </div>

            <span class="px-3 py-1 rounded-md bg-sky-50 text-sky-600 text-xs font-black border border-sky-100">
                {{ $subjects->total() ?? 0 }} môn học
            </span>

        </div>

        <!-- TABLE HEADER -->
        <div class="px-5 py-3 bg-slate-50 border-b border-slate-200">

            <div class="grid grid-cols-12 text-xs font-black text-slate-500 uppercase">

                <div class="col-span-5">
                    Môn học
                </div>

                <div class="col-span-2">
                    Tài liệu
                </div>

                <div class="col-span-2">
                    Giảng viên
                </div>

                <div class="col-span-2">
                    Trạng thái
                </div>

                <div class="col-span-1 text-right">
                    Hành động
                </div>

            </div>

        </div>

        <div id="subjects-area" class="divide-y divide-slate-100">

            @forelse($subjects as $subject)

            @php
            $documentCount = $subject->documents_count ?? 0;
            $teacherCount = $subject->lecturers->count();
            $isActive = $subject->status === 'active';

            /**
            * MAP MÀU AN TOÀN (KHÔNG DÙNG TAILWIND DYNAMIC)
            */
            $colorMap = [
            'blue' => ['bg' => 'bg-sky-50', 'text' => 'text-sky-600'],
            'green' => ['bg' => 'bg-emerald-50', 'text' => 'text-emerald-600'],
            'red' => ['bg' => 'bg-red-50', 'text' => 'text-red-600'],
            'yellow' => ['bg' => 'bg-yellow-50', 'text' => 'text-yellow-600'],
            'purple' => ['bg' => 'bg-purple-50', 'text' => 'text-purple-600'],
            ];

            $color = $subject->color ?? 'blue';
            $cls = $colorMap[$color] ?? $colorMap['blue'];
            @endphp

            <div class="px-5 py-4 hover:bg-slate-50 transition">
                <div class="grid grid-cols-12 items-center gap-4">

                    <!-- SUBJECT INFO -->
                    <div class="col-span-5 flex items-center gap-3 min-w-0">

                        <!-- ICON -->
                        <div class="w-9 h-9 rounded-md flex items-center justify-center shrink-0 {{ $cls['bg'] }}">
                            <i class="{{ $subject->icon ?? 'fa-solid fa-book' }} {{ $cls['text'] }} text-xs"></i>
                        </div>

                        <!-- TEXT -->
                        <div class="min-w-0">
                            <h3 class="font-black text-slate-700 truncate text-sm">
                                {{ $subject->subject_name }}
                            </h3>

                            <p class="text-xs text-slate-500 truncate">
                                {{ $subject->description ?? 'Không có mô tả' }}
                            </p>

                            <div class="flex gap-2 mt-1">
                                <span class="text-[10px] bg-slate-100 px-2 rounded">
                                    {{ $subject->subject_code }}
                                </span>

                                <span class="text-[10px] bg-slate-100 px-2 rounded">
                                    {{ $subject->faculty->faculty_name ?? 'Chưa có khoa' }}
                                </span>
                            </div>
                        </div>

                    </div>

                    <!-- DOCUMENT -->
                    <div class="col-span-2 font-black text-slate-700">
                        {{ $documentCount }}
                    </div>

                    <!-- TEACHER -->
                    <div class="col-span-2 font-black text-slate-700">
                        {{ $teacherCount }}
                    </div>

                    <!-- STATUS -->
                    <div class="col-span-2">
                        <span class="text-xs px-2 py-1 rounded font-black inline-flex items-center gap-1
                {{ $isActive ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-500' }}">

                            <span
                                class="w-1.5 h-1.5 rounded-full {{ $isActive ? 'bg-emerald-500' : 'bg-red-500' }}"></span>

                            {{ $isActive ? 'Hoạt động' : 'Ẩn' }}
                        </span>
                    </div>

                    <!-- ACTIONS (FIXED UI GIỐNG ẢNH) -->
                    <!-- ACTION -->
                    <div class="col-span-1 flex justify-end items-center">
                        <div class="flex items-center gap-2">

                            <!-- VIEW -->
                            <a href="{{ route('admin.subjects.show', $subject->subject_code) }}"
                                class="w-9 h-9 flex items-center justify-center rounded-md bg-sky-50 text-sky-600 hover:bg-sky-600 hover:text-white transition">

                                <i class="fa-solid fa-eye text-sm leading-none"></i>
                            </a>

                            <!-- EDIT -->
                            <a href="{{ route('admin.subjects.edit', $subject->subject_code) }}"
                                class="w-9 h-9 flex items-center justify-center rounded-md bg-amber-50 text-amber-500 hover:bg-amber-500 hover:text-white transition">

                                <i class="fa-solid fa-pen text-sm leading-none"></i>
                            </a>

                            <!-- DELETE -->
                            <form action="{{ route('admin.subjects.destroy', $subject->subject_code) }}" method="POST">
                                @csrf
                                @method('DELETE')

                                <button type="submit"
                                    class="w-9 h-9 flex items-center justify-center rounded-md bg-red-50 text-red-500 hover:bg-red-500 hover:text-white transition">

                                    <i class="fa-solid fa-trash text-sm leading-none"></i>
                                </button>
                            </form>

                        </div>
                    </div>
                </div>
            </div>

            @empty
            <div class="p-10 text-center text-slate-500">
                Không có môn học
            </div>
            @endforelse

        </div>
    </div>
    @endsection
    @push('scripts')
    <script>
    document.addEventListener('submit', function(e) {

        const form = e.target.closest('#subjects-filter-form');
        if (!form) return;

        e.preventDefault();

        const url = form.action + '?' + new URLSearchParams(new FormData(form));

        loadSubjects(url);
    });

    document.addEventListener('click', function(e) {

        const link = e.target.closest('.ajax-subject-page, #reset-subject-filter');
        if (!link) return;

        e.preventDefault();

        loadSubjects(link.href);
    });

    async function loadSubjects(url) {
        const area = document.getElementById('subjects-area');

        area.classList.add('opacity-50');

        const res = await fetch(url, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
            }
        });

        const html = await res.text();

        const doc = new DOMParser().parseFromString(html, 'text/html');
        const newArea = doc.querySelector('#subjects-area');

        if (newArea) {
            area.innerHTML = newArea.innerHTML;
            window.history.pushState({}, '', url);
        }

        area.classList.remove('opacity-50');
    }
    </script>
    @endpush